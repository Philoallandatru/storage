...
> **致 Hermes：** 一次性总结本次预研所有结果 + 整合 docs/ 目录下所有相关文档，
...
| # | 配置 | requests | 读取设备 P95 (Read Dev P95) | 写入设备 P95 (Write Dev P95) | 缓存命中 (Cache Hit) | SSD 利用率 (SSD util) | 状态 (Status) |
|---|---|---|---:|---:|---:|---:|---:|:---:|
...
| 配置 | 总操作数 (total ops) | 读 % (Read %) | 写 % (Write %) | Prefill % | Decode % | Evict % | 平均对象大小 (Mean obj size) | P95 对象大小 (P95 obj size) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
...
| 配置 | r/s 平均 (r/s avg) | wkB/s 平均 (wkB/s avg) | %util 平均 (%util avg) | %util P95 | 主导模式 |
|---|---:|---:|---:|---:|---:|---|
...
| 配置 | 读混合比例 (rwmixread) | 读取块大小分布 (read bssplit) | 写入块大小分布 (write bssplit) | 队列深度 iodepth（蒸馏，需替换） |
|---|---:|---|---|---:|
...
| 模型 | 平均对象大小 (Mean obj size, 混合) | P95 对象大小 (P95 obj size) |
|---|---:|---:|
...
## 🧭 后续测试方向（更新：2026-06-08）

### P0 优先级（关键）- **当前状态**

| 优先级 | 项 | 状态 |
|---|---|---|
| **P0 #1** | BurstGPT 70B CPU0 users 梯度 profile | ✅ Run1 (users=6)、Run2 (users=8) 完成 |
| **P0 #2** | prefill-only / decode-only 拆分 | ✅ Run4 (prefill)、Run5 (decode) 完成 |
| **P0 #3** | **fio iodepth sweep (32/64/128/256/1024)** | ✅ **已完成（2026-06-08）** |
| **P1 #1** | **SSD preconditioning sweep** | ✅ **已完成（2026-06-08）** |
| ~~P0 #4~~ | ~~object storage sweep~~ | ✅ 已完成（参见 MULTI_ENDPOINT_GUIDE.md） |

### P0 #3 fio sweep 完成结论（2026-06-08）

| Workload | 真实饱和 iodepth (Real Saturation iodepth) | 最大 IOPS (Max IOPS) | qd=1024 vs qd=32 的 R P99 恶化 |
|---|---|---|---|
| **sharegpt_8b_cpuhalf** | **qd=32** | 18,636 R / 11,909 W IOPS | 6.5ms → **167ms**（25×）|
| **burstgpt_8b_cpurel_spd1000** | qd>=32 稳定 | 21,065 R / 2,096 W IOPS | 3.5ms → **68ms**（19×）|
| **tp8_cpuhalf_generic** | qd=32 | 13,466 R / 4,980 W IOPS | 5.1ms → **156ms**（31×）|

**核心判断**：**这个 SSD 的真实饱和点在 qd=32**。蒸馏的原始 iodepth（524288 / 1048576）是 **system-side 队列深度**，不是 SSD 能处理的；超过 qd=64 之后增加 iodepth 只会**恶化延迟**而不提升 IOPS。**Write 路径在高 iodepth 下比 Read 严重 ~2.3 倍**（W P99 = 379ms vs R P99 = 167ms 在 sharegpt qd=1024 下）。

详细数据、饱和点表、4 面板曲线图见：
- 📄 `docs/kvcache-fio-iodepth-sweep-2026-06-08.md`（8.6KB 完整报告）
- 📊 `results/kvcache-profile/fio_sweep/sweep_summary.csv`（15 行 × 16 列原始数据）
- 📈 `results/kvcache-profile/fio_sweep/sweep_curves.png`（4 面板图）
- 📋 `results/kvcache-profile/fio_sweep/sweep_analysis.md`（自动生成的 md）

### P1 优先级（重要）

| 项 | 价值 | 备注 |
|---|---|---|
| **SSD preconditioning** | 高 | ✅ **已完成（2026-06-08）** — 见 `docs/kvcache-ssd-preconditioning-2026-06-08.md` |
| **长稳态 30-60 分钟** | 高 | 观察 GC 漂移、wear leveling、page cache 稳态 |
| **CPU cache sensitivity** | 中 | `cgroup` 限制 page cache 8/16/32GB 对比 |
| **object storage 端点对比** | 中 | s3dlio / minio / s3torchconnector 性能差 |
| **TP8 CPU1g / 2g 梯度** | 中 | 现有数据仅 CPU0.5g |

### P1 #1 SSD preconditioning 完成结论（2026-06-08）

| Workload | qd | R IOPS 变化 (R IOPS Δ) | R P99 变化 (R P99 Δ) | W IOPS 变化 (W IOPS Δ) | W P99 变化 (W P99 Δ) |
|---|---|---|---:|---:|---:|---:|
...
**核心判断**：
- **空盘数字偏乐观** — preconditioning 后所有 6 个 run 全部改善，有些指标改善惊人（W P99 -98%，R P99 -49%）
- **Write 路径改善最大** — GC 状态稳定后，write 不再被后台 GC 干扰，延迟降到裸设备水平
- **高 iodepth 改善更显著** — 空盘时高 iodepth 反而触发更多 GC；preconditioning 让高并发真正发挥并行优势
- **Read-mostly 工作负载改善最小** — burstgpt 91% 读，read 路径不经过 GC，所以 preconditioning 对它影响小

详细数据见：
- 📄 `docs/kvcache-ssd-preconditioning-2026-06-08.md`（10 KB 完整报告）
- 📊 `results/kvcache-profile/fio_sweep_precond/sweep_precond_summary.csv`（6 行 × 16 列）
- 📈 `results/kvcache-profile/fio_sweep_precond/sweep_precond_comparison.png`（4 面板对比图）

### P2 优先级（探索，未跑）

| 项 | 价值 | 备注 |
|---|---|---|
| 其他 LLM 模型 | 中 | Qwen3-32B / Mixtral-8x7B / DeepSeek-V3 |
| 多 SSD / RAID-0 | 高 | nvme0n1 + nvme2n1 + nvme3n1 串联，验证延迟是不是变 1/N |
| 真硬件延迟对比 | 高 | Samsung PM9A3 / Kioxia CD8P / ZHITAI TiPro9000 |
...
- 所有时间戳格式：`YYYYMMDD_HHMMSS`（例如 `20260608_113434`）
