# KV-Cache Long Steady-State Run — 30-Minute GC Drift Analysis

日期:2026-06-09
测试时长:30 分钟(1800s 实际 1810s)
测试配置:BurstGPT 70B users=6,real I/O mode, `--enable-latency-tracing`,autoscaling
测试盘: `/dev/nvme1n1` (BIWIN X570 1TB,3D TLC,steady state after SLC test)

## 测试目的

**核心问题**:长稳态下 SSD 行为如何演化?GC 是否在30 分钟内累积导致性能下降?

## 数据来源

| 类型 | 文件 | 大小 |
|---|---|---|
| iostat.log | `profiling/long_steady_state_30min_20260609_103815/iostat.log` | 1810 行(per-second) |
...
| 5分钟窗口 CSV | `long_steady_analysis/iostat_window_5min.csv` | 7 窗口 |
...
| 指标 | 值 |
|---|---:|
| **Avg Read BW** | **2082 MB/s** |
| Avg Write BW | 268 MB/s |
| Avg Read IOPS | 17,049 |
| Avg Write IOPS | 2,204 |
| **Avg %util** | **59.6%** |
| **Avg await** | **2.51 ms** |
| Peak %util | 79.7% |
| Peak await | 25.2 ms |
...
**Bench 累积 I/O**:
- Total Read: **3.61 TiB**
- Total Write: 425.6 GiB
- Read/Write Ratio: **8.48**
- Total Traced I/Os: **34,835,995** (30.85M reads,3.98M writes)

## 🧠 关键发现:显著 GC Drift

### 5分钟窗口对比

| 窗口 | R MB/s | W MB/s | R IOPS | W IOPS | %util | await | await P95 | await max |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
| 指标 | 早期 (0-300s) | 中期 (600-1200s) | 后期 (1500-1800s) | 变化 |
|---|---:|---:|---:|---:|
| Read BW (MB/s) | 2981 | ~2000 | **844** | **-72%** |
...
1. **Autoscaling ramp-down**:Bench 启动时 queue 持续增长 → autoscaler 把 user count 从 ~30 scale down 到 1。这是**主要驱动**:后期 I/O 量下降。

2. **GC drift(真实的 SSD 行为)**:即使 I/O 减少,**await 反而上升**(从 1.19 → 3.15 ms)。这是 GC 在稳态下持续工作,污染随机读路径。

3. **Wait queue 拥塞**:Bench 日志显示 Queue 从 0 → 376K(后期),说明 LLM 推理在等 SSD 响应。

### 为什么 avg Read BW 后期会下降?

不是 SSD 变慢,**是**:
- 用户数减少 → 写请求减少 → KV 缓存压力下降 → 读取需求下降
- BW 自然下降到低负载水平
- 但 **await 没有按比例下降** = SSD 单操作变慢
...
**对延迟敏感的 AI 工作负载**(interactive inference):
- 早期(0-300s)延迟低 (~1.2 ms) — 适合 SLA-critical workload
- 后期(>1200s)延迟高 (~3.2 ms) — 需要重新设计 SLA target
- **建议**:AI SSD vendor 应**同时报告 P95 延迟的早/晚值**,不应只给平均

### 3. LLM 推理有 Queue bottleneck
...
### 4. Read-dominant workload (R/W = 8.5)

LLM KV cache 是 **read-heavy** — 与 checkpointing (write-heavy) 完全不同。
**AI SSD 不应通用化**:需要专门的 read-optimized profile。
