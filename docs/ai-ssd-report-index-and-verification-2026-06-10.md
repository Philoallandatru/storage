...
| 优先级 | 报告 | 状态 | 用途 |
|---|---|---|---|
| 1 | `docs/ai-ssd-boss-report-2026-06-15.md` | 已修正结论边界 | 管理层版本入口，强调 KV Cache 当前只是上限压力测试 |
| 2 | `docs/ai-pc-agent-storage-workload-plan-2026-06-17.md` | 新增 | AI PC / Agent PC 场景扩展计划，覆盖 RAG、Agent memory、Code Agent、多模型、checkpoint |
| 3 | `docs/ai-ssd-kvcache-integrated-prestudy-report-2026-06-13.md` | 新增 | 6月13日整合大报告，作为 KV Cache 技术主入口 |
| 4 | `docs/ai-ssd-kvcache-complete-archive-report-2026-06-13.md` | 新增 | 完整实验归档，覆盖主线和支线测试 |
| 5 | `docs/kvcache-ai-ssd-product-prestudy-complete-2026-06-08.md` | 已修正 | KV Cache + AI SSD 产品预研总入口 |
| 6 | `docs/kvcache-saturation-points-2026-06-08.md` | 已修正 | 70B users12 / 8B users32 饱和边界 |
| 7 | `docs/kvcache-long-steady-state-2026-06-09.md` | 已修正措辞 | 30 分钟长稳态和 GC drift |
| 8 | `docs/cross-vendor-nvme-comparison-2026-06-09.md` | 已修正 caveat | 四盘横向对比 |
| 9 | `docs/biwin-x570-ssd-characterization-2026-06-08.md` | 已核验 | BIWIN X570 Gen5/TLC-like/SLC cache 基础判断 |
| 10 | `docs/biwin-x570-slc-steady-state-vs-fresh-2026-06-09.md` | 可用，需单次样本 caveat | BIWIN fresh vs steady SLC cache |
| 11 | `docs/biwin-x570-slc-mixed-rw-2026-06-09.md` | 可用，需单次样本 caveat | mixed R/W 下 SLC cache 价值 |
| 12 | `docs/kvcache-pagecache-sensitivity-2026-06-09.md` | 可用，需 order-effect caveat | DRAM page cache 敏感性 |
| 13 | `docs/ai-ssd-multidisk-validation-plan-2026-06-10.md` | 新增 | 多盘对比设计与 AI SSD 产品验证计划 |
...
3. 当前 KV Cache forced-NVMe 结果应定位为 upper-bound stress test，不能直接外推为完整生产用户场景或最终采购结论。
4. 70B TP8 CPU0 BurstGPT 是比 8B 更有价值的产品边界 workload，但仍需 production-like tiering、低 speedup、bounded cache 和 3-run median。
5. BIWIN X570 是强 burst 盘；Seagate FC530 在当前单盘 KV 长稳态压测中写 tail 更稳；mixed checkpoint + inference 仍需直接组合 workload 验证。
6. mixed R/W 场景下 SLC cache burst 价值明显下降，AI SSD 产品不能只宣传顺序写入 SLC 峰值。
7. 下一阶段必须扩展到 AI PC / Agent PC 场景：Agent memory/RAG、Code Agent、小文件/metadata、模型切换、checkpoint 与前台推理互扰。
8. 后续对候选 AI SSD 应固定报告：fresh/steady SLC、mixed R/W、KV object P95/P99、long steady GC drift、page cache sensitivity、70B service boundary、Agent/RAG/Code workload。
